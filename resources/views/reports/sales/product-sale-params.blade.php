...
@section('content')
    <div class="container-fluid">
        ...
        <div class="param-card">
            ...
            <form id="paramForm"
                  action="{{ route('reports.product-sale.generate') }}"
                  method="GET"
                  target="_blank">
                ...
                <div class="param-body">
                    ...
                    {{-- ── 2. Salesman & Area ── --}}
                    <div class="group-label">Salesman &amp; Area</div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="field-label" for="salesmanSel">
                                <i class="bi bi-person-badge"></i> Salesman
                            </label>
                            <select id="salesmanSel" name="salesman_code" class="form-select">
                                <option value="">— All Salesmen —</option>
                                @foreach ($salesmen as $sm)
                                    <option value="{{ $sm->salesman_code }}">{{ $sm->salesman_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="field-label" for="areaSel">
                                <i class="bi bi-geo-alt"></i> Sales Area
                            </label>
                            <select id="areaSel" name="area_code" class="form-select">
                                <option value="">— No Areas Filter —</option>
                                @foreach ($areas as $area)
                                    <option value="{{ $area->area_code }}">{{ $area->description }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="group-label">
                        Customer
                        <span class="count-badge" id="custCount">{{ $customers->count() }} records</span>
                    </div>
                    <div class="mb-4">
                        <label class="field-label" for="customerSel">
                            <i class="bi bi-building"></i> Customer
                        </label>
                        <div class="sel-wrap">
                            <select id="customerSel" name="debtor_no" class="form-select">
                                <option value="">— All Customers —</option>
                                @foreach ($customers as $cust)
                                    <option value="{{ $cust->debtor_no }}">{{ $cust->name }}</option>
                                @endforeach
                            </select>
                            <span class="spinner-border spinner-border-sm sel-spin text-primary" id="custSpinner"></span>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        (function () {
            ...
            /* ── Salesman / Area → Customer AJAX cascade ─────────────────────── */
            ...
            const custSpinner = document.getElementById('custSpinner');
            const countBadge  = document.getElementById('custCount');
            const allCustOpts = Array.from(customerSel.options).map(o => ({ v: o.value, t: o.text }));
            ...
            function rebuildCustomers(list) {
                ...
                const total = list.filter(i => (i.v ?? i.debtor_no) !== '').length;
                countBadge.textContent = total + ' records';
            }

            function fetchCustomers() {
                const code = salesmanSel.value;
                const area = areaSel.value;

                if (!code && !area) {
                    rebuildCustomers(allCustOpts);
                    return;
                }

                custSpinner.classList.add('on');
                customerSel.disabled = true;

                const params = new URLSearchParams();
                if (code) params.append('salesman_code', code);
                if (area) params.append('area_code', area);

                fetch(`{{ route('reports.product-sale.customers') }}?${params}`, {
                    ...
                    .catch(() => rebuildCustomers(allCustOpts))
                    .finally(() => { custSpinner.classList.remove('on'); customerSel.disabled = false; });
            }

            salesmanSel.addEventListener('change', fetchCustomers);
            areaSel.addEventListener('change', fetchCustomers);
